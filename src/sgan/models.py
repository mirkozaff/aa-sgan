...
class SocialPooling(nn.Module):
    ...
    def forward(self, h_states, seq_start_end, end_pos):
        ...
        for _, (start, end) in enumerate(seq_start_end):
            ...
            within_bound = within_bound.contiguous().view(-1)
            ...
            offset = self.repeat(offset.contiguous().view(-1, 1), num_ped).contiguous().view(-1)
            grid_pos += offset
            grid_pos[within_bound != 0] = 0
            grid_pos = grid_pos.contiguous().view(-1, 1).expand_as(curr_hidden_repeat)

            curr_pool_h = curr_pool_h.scatter_add(0, grid_pos,
                                                  curr_hidden_repeat)
            curr_pool_h = curr_pool_h[1:]
            pool_h.append(curr_pool_h.contiguous().view(num_ped, -1))
    ...
    def forward(self, obs_traj, obs_traj_rel, seq_start_end, user_noise=None):
        ...
        if self.pooling_type:
            ...
            mlp_decoder_context_input = torch.cat(
                [final_encoder_h.contiguous().view(-1, self.encoder_h_dim), pool_h], dim=1)
        else:
            mlp_decoder_context_input = final_encoder_h.contiguous().view(
